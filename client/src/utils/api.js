...
const MCP_URL = import.meta.env.VITE_MCP_URL || 'https://mcp-server-and-langgraph-agent-production.up.railway.app/mcp';

const getAuthToken = () => {
  return import.meta.env.VITE_MCP_TOKEN || '';
};
...
export const kickstartGenerateProposalFromText = async ({ report_text }, options) => {
  return mcpToolsCall('kickstart_generate_proposal_from_text', { report_text }, options);
};

export const kickstartCreateProposal = async (
  proposal,
  options,
) => {
  return mcpToolsCall(
    'kickstart_create_proposal',
    { ...proposal },
    options,
  );
};
...
export const kickstartGenerateProposalAi = async (
  ...
  return mcpToolsCall('kickstart_generate_proposal_ai', { proposal_id, prompt, sections }, options);
};
